...
/**************宏定义******************************/
#define	  	D_EDIT_FCOLOR 		Red
#define	  	D_EDIT_BCOLOR 		Grey
	...
	Y = GUI_Edit_P->Y+2;	
		...
		{//EDIT_STATE_COLORTYPE/*焦点选择的位置大于0 总条件*/	//不是需要EDIT
			...
			{							
				...
				{
					X +=GUI_API.Font->ASC_X;
					GUI_PutChar(GUI_Edit_P->String[i],X,Y,FColor,BColor,GUI_Edit_P->Font);
				}
				else
				{
					X +=GUI_API.Font->GB_X;					
					GUI_PutGBChar((u8*)GUI_Edit_P->String+i,X,Y,FColor,BColor,GUI_Edit_P->Font);
					i++;
				}
			}
			else
...
void GUI_EditShowNow(GUI_EditStruct *GUI_Edit_P)						   //
{
	...
	//画edit外框		
		...
		{
			GUI_DrawRectangle(GUI_Edit_P->X,GUI_Edit_P->Y,GUI_Edit_P->X+GUI_Edit_P->MaxLen*8+3,GUI_Edit_P->Y+19,0xFFFF-GUI_API.B_Color);
		}
		else
		{
			GUI_DrawRectangle(GUI_Edit_P->X,GUI_Edit_P->Y,GUI_Edit_P->X+GUI_Edit_P->MaxLen*8+3,GUI_Edit_P->Y+19,Black);			
		}
		GUI_DrawRectangle(GUI_Edit_P->X+1,GUI_Edit_P->Y+1,GUI_Edit_P->X+GUI_Edit_P->MaxLen*8+2,GUI_Edit_P->Y+18,Black);		
	//显示EDIT数据
...
void GUI_EditKeyFirst(GUI_EditStruct *GUI_Edit_P)
{
	if(!GUIGetBit(GUI_Edit_P->State,GUI_DIS))			//控件是否是使能状态
	{
		switch(UintToByte0(GUI_API.KeyState))
		{
		case GUIKEY_UP://向上
...
void GUI_EditTouchUp(GUI_EditStruct *GUI_Edit_P)
{
	...
	{
//		GUI_EditUseKeyEsc(GUI_Edit_P);
		GUI_API.TouchChange = TOUCHNULL;
		GUI_API.CurrentForm->TouchNum = NOTOUCHNUM;	
	}
}
/********************************************
...
u8 GUI_EditTouchDown(GUI_EditStruct *GUI_Edit_P,u8 Num)
{
	...
	if(!GUIGetBit(GUI_Edit_P->State,GUI_DIS))										//按钮不是DIS状态		
		{													
			if((GUI_API.Touch_X >X1)&&(GUI_API.Touch_Y>Y1)&&(GUI_API.Touch_X<X2)&&(GUI_API.Touch_Y<Y2))		 //判断是否在按钮内部
			{	
				GUI_EditUseKeyEnter(GUI_Edit_P);
				GUIResetBit(GUI_Edit_P->State,GUI_SHOWOK);			//控件需重画
//				GUI_API.CurrentForm->FocusID =  Num+1;
//				GUI_API.CurrentForm->FocusID =  GUI_CurrenFormGetIDNum((u8*)(GUI_Edit_P));				
				GUI_API.CurrentForm->TouchNum = Num;
				GUI_API.CurrentForm->FocusIsSelect = true;
				GUI_API.TouchChange = TOUCHNULL;
