...
#if USE_MSOS
	#define	SDMutex						TaskStatusSuspend7
	#define SD_LOCK()					OS.OSMutexLock(SDMutex)	
	#define SD_UNLOCK()				OS.OSMutexUnLock(SDMutex)
...
SD_Error SD_ReadBlock_1(u8 *buf,long long addr,u16 blksize)
{	  
	SD_Error errorstatus=SD_OK;
	u8 power;
...
{	
	...
	do
  {
	...
	Err = SD_ReadBlock_1(buf,addr,blksize) ; 	
  	i++;
		if (i >=0x3FFFF) return Err;
  } while (Err);
  return Err;
}	
//SD卡读取多个块 
