...
#define	  	D_KeyBoard_BCOLOR 		Red
#define	  	D_KeyBoard_FCOLOR 		Grey
#define	  	D_KeyBoard_SELECTCOLOR 		Blue
...
{
	u32 KeyMessage;
	if(KeyBoard_P->ID != GUIKeyBoardID)		return false;	
	if(!GUIGetBit(KeyBoard_P->State,GUI_DIS))		//不是DISABLE
	{
		if(GUIGetBit(KeyBoard_P->State,KeyBoard_TOUCH))		//按键被按下
		{	
			KeyMessage = ((u32)KeyMessageType<<24) | ((KeyBoard_P->SelectNum | KeyBoard_P->KeyID)| KEYOFF<<8);
			#if GUI_USEMSOS
...
u8 GUI_KeyBoardTouchLong(GUI_KeyBoardStruct *KeyBoard_P)
{
	...
	if(!GUIGetBit(KeyBoard_P->State,GUI_DIS))		//不是DISABLE
	{
		if(GUIGetBit(KeyBoard_P->State,KeyBoard_TOUCH))		//按键被按下
		{
			KeyMessage = ((u32)KeyMessageType<<24) | ((KeyBoard_P->SelectNum | KeyBoard_P->KeyID)| KEYLONG<<8);
			#if GUI_USEMSOS
					...
					if(T_Y < Y)
					{
						...
						for (j=0;j<KeyBoard_P->X_Num;j++)	
						{
							...
							{						
								...
								KeyMessage = ((u32)KeyMessageType<<24) | ((KeyBoard_P->SelectNum | KeyBoard_P->KeyID)| KEYFIRST<<8);
								#if GUI_USEMSOS
...
//{
//	if(GUI_API.KeyBoard_P == null)			return ;
//	if(GUI_API.KeyBoard_P->ID != GUIKeyBoardID) 	return ;				//不是KeyBoard的数据
//	GUIResetBit(GUI_API.KeyBoard_P->State,GUI_FOCUS);									//控件取消系统焦点
//	GUIResetBit(GUI_API.KeyBoard_P->State,KeyBoard_FOCUSIN);						//控件取消内部焦点	
//	GUI_API.KeyBoard_P->SelectNum =0;																	//功能键选择归零
//	GUIResetBit(GUI_API.KeyBoard_P->State,GUI_SHOWOK);									//控件显示有变化			
//}
...
//	if(GUI_API.KeyBoard_P == null)			return ;
//	if(GUI_API.KeyBoard_P->ID != GUIKeyBoardID) 	return ;				//不是KeyBoard的数据
//	GUISetBit(GUI_API.KeyBoard_P->State,GUI_FOCUS);										//控件获取系统焦点
//	GUIResetBit(GUI_API.KeyBoard_P->State,KeyBoard_FOCUSIN);						//控件取消内部焦点	
//	GUI_API.KeyBoard_P->SelectNum =0;																	//功能键选择归零
//	GUIResetBit(GUI_API.KeyBoard_P->State,GUI_SHOWOK);									//控件显示有变化			
//}
