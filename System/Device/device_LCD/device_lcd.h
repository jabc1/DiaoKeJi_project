...
#include "os.h"
	...
	#define LCD_LOCK()					OS.OSMutexLock(GUIMutex)	
	#define LCD_UNLOCK()				OS.OSMutexUnLock(GUIMutex)
#else
...
/* 整理后3个LCD共用的函数声明*/
void InitializeLcd(void);																					//LCD初始化
void LCD_BackLight(u8 status);																		//背光控制
void LCD_BeginWriteRAM(void);																			//LCD开始写入RAM
void LCD_EndWriteRAM(void);																				//LCD结束写入RAM
void LCD_OnlyWriteRAM(u16 LCD_DATA);															//LCD仅仅写入RAM,没有控制的命令
void LCD_SetCursor(u16 x,u16 y);																	//设置屏幕座标
void LCD_SetPoint(u16 x,u16 y,u16 Color);													//LCD打点函数
void LCD_SetWindows(u16 StartX,u16 StartY,u16 EndX,u16 EndY);			//设置LCD屏幕范围

//1.44屏由于没有读取的IO，故读取函数没有
