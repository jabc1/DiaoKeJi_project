...
{
	DK.State_B_P = (DK_B_StateStruct *)BitAddr(&(DK.State));	//状态的位（位带操作）
	DK.Alarm_B_P = (DK_B_Alarm_Struct* )BitAddr(&(DK.Alarm));		//报警的位（位带操作）	
	LoadSYSData();								//读取系统参数
	InitializeGCodeQueue();				//初始化GCODE队列
	Moto_Limit_Init();						//限位初始化
	Initialize_Moto();						//电机初始化
...
int main(void) 
{
	...
	NRFTaskPriority = OS.CreateTask(NRFTask); //创建任务，优先级0 	
